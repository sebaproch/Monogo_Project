...
const markets = {
  ...
  //   confirmRemovingLocator: '//button[@data-testid="remove-item-submit-button"]',
  //   emptyCartLocator: '//*[contains(text(), "You have no items in your shopping cart at the moment.")]',
  // },
  PL: {
    ...
    confirmRemovingLocator: '//button[@data-testid="remove-item-submit-button"]',
    emptyCartLocator: '//*[contains(text(), "W tym momencie Twój koszyk jest pusty.")]',
...
function getMarketSelectors(countryCode: any) {
  ...
  for (const market in markets) {
    test(`Verifying adding product to cart in ${market} market`, async ({
      ...
      basketCountValue = parseInt(await page.inputValue(locators.cartQuantityLocator));
      ...
      if (market == "UK") {
        expect(emptyCartText).toEqual("You have no items in your shopping cart at the moment.")
      } else {
